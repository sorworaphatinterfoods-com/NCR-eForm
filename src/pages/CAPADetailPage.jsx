...
import { Save, ArrowLeft, Printer, ClipboardList, Link2, Check } from 'lucide-react'
...
export default function CAPADetailPage() {
  ...
  const replyLink = `${window.location.origin}/capa-reply/${id}`
  const copyReplyLink = async () => {
    try {
      await navigator.clipboard.writeText(replyLink)
    } catch {
      const t = document.createElement('textarea')
      t.value = replyLink; document.body.appendChild(t); t.select()
      ...
      <div className="max-w-4xl mx-auto px-4 py-4 pb-24 sm:pb-6">
        {loading ? (
          ...
          <>
            ...
            {!isNew && (
              <div className="mb-4 bg-teal-50 border border-teal-200 rounded-xl p-3 flex items-center gap-3">
                <Link2 className="w-5 h-5 text-teal-600 shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium text-teal-800">ลิงก์ตอบกลับสำหรับผู้ส่งมอบ (CAPA)</div>
                  <div className="text-xs text-teal-600 truncate">{replyLink}</div>
                </div>
                <button
                  onClick={copyReplyLink}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition shrink-0 ${copied ? 'bg-green-600 text-white' : 'bg-teal-600 hover:bg-teal-700 text-white'}`}
                >
